...
var initCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := remote.RunParallelScript([]string{host}, AppConfig.SSHUser, AppConfig.SSHKey, "init.sh", []string{"kubeadm-config.yaml", "tokens.csv", "wait-for-master.sh"}); err[0] != nil {
			return err[0]
		}
